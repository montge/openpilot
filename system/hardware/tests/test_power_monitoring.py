...
from openpilot.system.hardware.power_monitoring import (
  PowerMonitoring,
...
class TestPowerMonitoring:
    ...
    for i in range(TEST_TIME):
      ...
      if i % 10 == 0:
        assert pm.should_shutdown(ignition, True, start_time, True) == (
          pm.car_voltage_mV < VBATT_PAUSE_CHARGING * 1e3
          and (ssb - start_time) > VOLTAGE_SHUTDOWN_MIN_OFFROAD_TIME_S
          and (ssb - start_time) > DELAY_SHUTDOWN_TIME_S
        )
    assert pm.should_shutdown(ignition, True, start_time, True)
  ...
  def test_delay_shutdown_time(self):
    ...
    assert pm.should_shutdown(ignition, in_car, offroad_timestamp, started_seen), f"Should shutdown after {DELAY_SHUTDOWN_TIME_S} seconds offroad time"

  def test_should_shutdown_none_offroad_timestamp(self):
    """Test should_shutdown returns False when offroad_timestamp is None."""
    pm = PowerMonitoring()
    pm.car_battery_capacity_uWh = 0
    pm.car_voltage_mV = VOLTAGE_BELOW_PAUSE_CHARGING

    # Even with all other conditions met, None offroad_timestamp returns False
    assert pm.should_shutdown(ignition=False, in_car=True, offroad_timestamp=None, started_seen=True) is False

  def test_calculate_handles_exception(self, mocker):
    """Test calculate handles exceptions gracefully."""
    pm = PowerMonitoring()
    # First call to set last_measurement_time
    pm.calculate(GOOD_VOLTAGE, False)

    # Mock HARDWARE.get_current_power_draw to raise an exception
    mocker.patch("openpilot.system.hardware.power_monitoring.HARDWARE.get_current_power_draw", side_effect=RuntimeError("Test exception"))
    mock_cloudlog = mocker.patch("openpilot.system.hardware.power_monitoring.cloudlog")

    # Should not raise, should log exception
    pm.calculate(GOOD_VOLTAGE, False)

    # Verify cloudlog.exception was called
    mock_cloudlog.exception.assert_called_once()

  def test_perform_integration_handles_exception(self, mocker):
    """Test _perform_integration handles exceptions gracefully."""
    pm = PowerMonitoring()
    pm.last_measurement_time = 100.0

    mock_cloudlog = mocker.patch("openpilot.system.hardware.power_monitoring.cloudlog")

    # Mock time to go backwards (negative integration time triggers ValueError path)
    # But the exception is caught, so we check cloudlog was called
    pm._perform_integration(50.0, 4.0)  # t < last_measurement_time causes negative

    # The ValueError is raised and caught
    mock_cloudlog.exception.assert_called_once()
