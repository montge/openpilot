...
from openpilot.selfdrive.locationd.helpers import (
  ...
  Measurement, Pose, PoseCalibrator,
)
...
class TestFftNextGoodSize(unittest.TestCase):
  ...
  def test_power_of_two(self):
    """Test powers of two."""
    self.assertEqual(fft_next_good_size(8), 8)
    self.assertEqual(fft_next_good_size(16), 16)

  def test_composite_of_small_primes(self):
    """Test composites of small primes."""
    # 12 = 2^2 * 3
    self.assertEqual(fft_next_good_size(12), 12)
    # 15 = 3 * 5
    self.assertEqual(fft_next_good_size(15), 15)
  ...
  def test_finds_next_good_size(self):
    ...
    self.assertGreaterEqual(result, 13)
    # Should be a composite of 2, 3, 5, 7, 11
    self.assertEqual(result, 14)  # 2 * 7
...
class TestNPQueue(unittest.TestCase):
  ...
  def test_arr_shape(self):
    ...
    self.assertEqual(q.arr.shape, (2, 5))
...
class TestPointBuckets(unittest.TestCase):
  ...
  def test_get_points_empty(self):
    ...
    # This will fail if buckets are empty - let's add some points first
    x_bounds = [(0.0, 10.0)]
    min_points = [1]
    buckets2 = PointBuckets(
      x_bounds=x_bounds,
      min_points=min_points,
      min_points_total=1,
      points_per_bucket=10,
...
class TestMeasurement(unittest.TestCase):
  ...
  def test_from_measurement_xyz(self):
...
class TestPose(unittest.TestCase):
  ...
  def test_init(self):
    ...
    self.assertEqual(pose.velocity, velocity)
    self.assertEqual(pose.acceleration, acceleration)
    self.assertEqual(pose.angular_velocity, angular_velocity)
