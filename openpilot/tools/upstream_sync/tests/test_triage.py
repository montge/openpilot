...
class TestTriage:
  ...
  def test_cli_facts_only(self, repo, monkeypatch):
    monkeypatch.chdir(repo)  # the CLI triages the repository it runs in
    assert main(["--fork", "develop", "--upstream", "master", "--out", "out", "--facts-only"]) == 0
    assert (repo / "out" / "triage.md").exists() and (repo / "out" / "triage.json").exists()

  def test_cli_rejects_paths_outside_working_tree(self, repo, monkeypatch):
    monkeypatch.chdir(repo)
    with pytest.raises(SystemExit):
      main(["--fork", "develop", "--upstream", "master", "--out", "../escape", "--facts-only"])
