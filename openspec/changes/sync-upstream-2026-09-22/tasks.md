...
- [x] 4.4 CI on PR #62 (final head `fcebcd057`): 24 passed, 1 skipped (simulator), 1 failed (SonarCloud quality gate, below).
      - **`unit tests`: 3786 passed, 176 skipped, 1 xfailed, 0 failed.** The 10 macOS-local failures, `test_logmessaged` included, were all environmental. One run hit a flaky truncated route download (`test_dataloader.py::test_extract_model_outputs`, bz2 "Compressed data ended before the end-of-stream marker"); it passed on re-run and on the two other heads.
...
- [x] 4.5 SonarCloud now really scans: `SONAR_TOKEN` was regenerated, closing September follow-up 6.5's token half. Its PR quality gate failed on:
      - **Security rating C on new code:** five vulnerabilities.
        ...
        - Three remain, all in upstream-owned files: `scripts/lint/check_shell.py` (the same two rules) and `scripts/apply-pr.sh` (`curl -L` without HTTPS enforcement), both arriving with #38877. Left untouched to keep the fork diff at zero, so the rating stays C until they are accepted in SonarCloud or fixed upstream (see 6.4).
      - **20.5% coverage on new code (≥80% required).** Structural for sync PRs: "new code" includes every upstream line the merge brings in, which the fork's tests do not target.
...
- [ ] 6.4 SonarCloud policy for sync PRs (the user's call, in SonarCloud or `sonar-project.properties`): accept the three upstream-owned findings (or send them upstream as a PR), and decide how the new-code coverage condition should treat merge commits from upstream (e.g. a "previous version" new-code period, or reviewing the gate on develop after the merge rather than on the PR)
